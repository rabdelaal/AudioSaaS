...
def run_ffmpeg(args):
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"ffmpeg failed: {proc.stderr.decode('utf8', errors='ignore')}")
    return proc
...
def run_mmaudio_job(job_id: str, video_path: str, prompt: str, duration: float, variant: str):
    """Run the MMAudio demo.py as a subprocess and update JOBS with progress and outputs.

    This will invoke the demo script in the cloned MMAudio repo. It assumes dependencies
    (PyTorch, torchaudio, etc.) are installed and models will be downloaded automatically.
    """
    ...
    cmd = [sys.executable, os.path.join(BASE_DIR, 'MMAudio', 'demo.py'),
           '--video', video_path,
           '--prompt', prompt,
...
def _ffprobe_duration(path: str) -> float:
    try:
        proc = subprocess.run(['ffprobe', '-v', 'error', '-show_entries', 'format=duration', '-of', 'default=noprint_wrappers=1:nokey=1', path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out = proc.stdout.decode('utf8', errors='ignore').strip()
...
def run_pipeline_job(job_id: str, video_path: str, options: dict):
    ...
    # desired components
    foley_engine = options.get('foley', 'foleycrafter')
    ...
    # If TTS chosen but not available, and pyttsx3 not available either, ask user to install
    ...
    if missing:
        JOBS[job_id]['status'] = 'error'
        JOBS[job_id]['error'] = ('Missing required packages: ' + ', '.join(missing) +
                                '. Install them and retry. Example commands:\n' +
                                "pip install whisperx pyttsx3\n# For Bark: see https://github.com/suno-ai/bark\n# For Parler-TTS: check its repo and install instructions")
        return

    # If user requested the minimal reference backend, ensure it's available
    if backend == 'ref' and not REF_IMPL_OK:
        JOBS[job_id]['status'] = 'error'
        JOBS[job_id]['error'] = 'Reference backend (ref_impl) not available. Ensure ref_impl package is in the repo.'
        return
    ...
    if has_audio_stream(video_path):
        ...
        try:
            run_ffmpeg(['ffmpeg', '-y', '-i', video_path, '-vn', '-acodec', 'copy', extracted_audio])
        except RuntimeError:
            # re-encode fallback
            run_ffmpeg(['ffmpeg', '-y', '-i', video_path, '-vn', '-ac', '2', '-ar', '44100', extracted_audio])
    ...
    try:
        ...
        run_ffmpeg(['ffmpeg', '-y', '-f', 'lavfi', '-i', f"anoisesrc=color=pink:duration={duration}", '-ar', '44100', '-ac', '2', foley_path])
    except Exception as e:
        JOBS[job_id]['status'] = 'error'
            ...
            if bark_ok:
                try:
                    ...
                    try:
                        ...
                        write_wav(tts_path, bark_gen.SAMPLE_RATE, audio_arr.astype('int16') if audio_arr.dtype != 'int16' else audio_arr)
                    except Exception:
    ...
    try:
        ...
        for m in mix_inputs:
            inputs += ['-i', m[1]]
        amix_cmd = ['ffmpeg', '-y'] + inputs + ['-filter_complex', f"amix=inputs={len(mix_inputs)}:normalize=1", '-ar', '44100', '-ac', '2', final_audio]
        run_ffmpeg(amix_cmd)
    except Exception as e:
        JOBS[job_id]['status'] = 'error'
    ...
    try:
        run_ffmpeg(['ffmpeg', '-y', '-i', video_path, '-i', final_audio, '-c:v', 'copy', '-c:a', 'aac', '-shortest', out_video])
    except Exception as e:
        JOBS[job_id]['status'] = 'error'
    ...
    checkpoint_paths = [os.path.join(wav2lip_dir, 'checkpoints', 'wav2lip_gan.pth'),
    ...
    if available_ckpt and os.path.isdir(wav2lip_dir):
        ...
        cmd = [sys.executable, os.path.join(wav2lip_dir, 'inference.py'),
               '--checkpoint_path', available_ckpt,
               '--face', video_path,
               '--audio', final_audio,
               '--outfile', lipsync_out]
        try:
            ...
            if proc.returncode != 0:
                ...
                JOBS[job_id]['outputs'] = [os.path.basename(final_audio), os.path.basename(lipsync_out)]
                JOBS[job_id]['status'] = 'done'
                return
        except Exception as e:
        ...
        if os.path.isdir(wav2lip_dir):
            JOBS[job_id]['progress_msg'] = (
                'Wav2Lip checkpoint missing. Download the checkpoint (wav2lip_gan.pth or wav2lip.pth) '
                'and place it in Wav2Lip/checkpoints/')

    JOBS[job_id]['status'] = 'done'
    JOBS[job_id]['outputs'] = [os.path.basename(final_audio), os.path.basename(out_video)]
...
def start_pipeline():
    ...
    opts = {'foley': foley, 'tts': tts, 'sync': sync, 'transcript': transcript, 'duration': request.form.get('duration')}
    t = threading.Thread(target=run_pipeline_job, args=(job_id, video_path, opts), daemon=True)
    t.start()
...
def process():
    # Checklist: receive files, save them, extract or generate audio, merge, provide download
    if 'video' not in request.files:
    ...
    try:
        if audio and audio.filename != '':
            ...
            audio_path = os.path.join(UPLOAD_DIR, saved_audio)
            args = [
                'ffmpeg', '-y', '-f', 'lavfi', '-i', f"sine=frequency=440:duration={gen_duration}",
            ...
            if has_audio_stream(video_path):
                ...
                try:
                    args = ['ffmpeg', '-y', '-i', video_path, '-vn', '-acodec', 'copy', audio_path]
                    run_ffmpeg(args)
                except RuntimeError:
                    args = ['ffmpeg', '-y', '-i', video_path, '-vn', '-ac', '2', '-ar', '44100', audio_path]
                    run_ffmpeg(args)
            else:
                # No audio stream present. Fall back to generating a short sine tone so merging succeeds.
                saved_audio = f"{uid}_generated_from_noaudio.wav"
                audio_path = os.path.join(UPLOAD_DIR, saved_audio)
                gen_dur = max(1.0, float(request.form.get('generate_duration') or 10))
                args = [
                    'ffmpeg', '-y', '-f', 'lavfi', '-i', f"sine=frequency=440:duration={gen_dur}",
                    '-ar', '44100', '-ac', '2', audio_path
                ]
                run_ffmpeg(args)
...
def start_job():
    """Start a MMAudio generation job using the cloned repo's demo.py script.

    Expects multipart form with `video` (file) and `prompt`, optional `duration` and `variant`.
    Returns job id as JSON.
    """
    if 'video' not in request.files:
        return {'error': 'no video uploaded'}, 400
    video = request.files['video']
    ...
    job_id = uuid.uuid4().hex
    JOBS[job_id] = {'status': 'queued', 'video': saved_video, 'prompt': prompt, 'created_at': time.time()}

    t = threading.Thread(target=run_mmaudio_job, args=(job_id, video_path, prompt, duration, variant), daemon=True)
    t.start()
